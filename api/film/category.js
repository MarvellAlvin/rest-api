...
module.exports = async (req, res) => {
    ...
    const { category, page = 1 } = req.method === 'GET' ? req.query : req.body;
    ...
    if (!category) {
        return res.status(400).json({
            ...
            statusCode: 400,
            author: '@velz',
            error: 'Parameter "category" wajib diisi. Contoh: ongoing, box-office, drama-serial-korea',
    ...
    try {
        ...
        const cleanPath = category.replace(/^\/|\/$/g, '');
        const baseCategoryUrl = `${baseDomain}/kategori-film/${cleanPath}`;
        const targetUrl = (page > 1) ? `${baseCategoryUrl}/page/${page}/` : baseCategoryUrl;
